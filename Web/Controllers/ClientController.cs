...
namespace Web.Controllers
{
    public class ClientController : Controller
    {
        ...
        public ActionResult FrpConfig(int UId, int Id)
        {
            string Json;
            DLL.BLL.FRPConfigBLL fb = new DLL.BLL.FRPConfigBLL();
            if (Id == 0)
            {
                Json = JsonConvert.SerializeObject(fb.SelectCommonFrpConfig());

            }
            else
            {
                Json = JsonConvert.SerializeObject(fb.SelectUsersFrpConfig(UId));
